            ...
            <div class="card">
                ...
                <div class="tab-nav overflow-x-auto">
                    ...
                    @foreach($adminTabs as $tabKey => $tabInfo)
                        <a class="tab-link flex items-center gap-2 whitespace-nowrap {{ $tab === $tabKey ? 'tab-link-active' : 'tab-link-inactive' }}" href="{{ route('admin.index', ['tab' => $tabKey]) }}">
                ...
                <div class="p-6">
                            ...
                            <table class="min-w-full text-sm">
                                <thead class="bg-slate-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Active</th>
                                        ...
                                        <tr class="hover:bg-slate-50/50">
                                            ...
                                            <td class="px-4 py-3">
                                                ...
                                                <label class="relative inline-flex items-center cursor-pointer">
                                                    <input type="checkbox" name="is_active" value="1" form="{{ $formId }}" @checked($u->is_active) class="sr-only peer" />
                                            ...
                                            <td class="px-4 py-3 text-slate-500 text-xs">{{ $u->last_login_at?->diffForHumans() ?? 'Never' }}</td>
                                        ...
                                        <div class="rounded-xl bg-slate-50 ring-1 ring-slate-900/5 p-4">
                                            <div class="flex items-start justify-between gap-4">
                                                <div class="flex-1">
                                                    ...
                                                    @if($p->base_url)
                                                        <div class="text-xs text-slate-400 mt-1 truncate">{{ $p->base_url }}</div>
                                                    @endif
                                                </div>
                                                <div class="flex items-center gap-3">
                                                    @if($p->last_tested_at)
                            ...
                            <div>
                                <div class="rounded-xl ring-1 ring-slate-900/5 overflow-hidden">
                                    <div class="px-5 py-4 bg-slate-50 border-b border-slate-100">
                                        ...
                                        <p class="text-xs text-slate-500 mt-0.5">OpenAI-compatible, Ollama, LM Studio, or GLM 4.7</p>
                                    </div>
                            ...
                            <div>
                                <div class="rounded-xl ring-1 ring-slate-900/5 overflow-hidden">
                                    <div class="px-5 py-4 bg-slate-50 border-b border-slate-100">
                                        ...
                                        <p class="text-xs text-slate-500 mt-0.5">Upload an HRP-503c .docx for content control scanning.</p>
                                    </div>
                    ...
                    @elseif($tab === 'settings')
                        ...
                        <p class="text-sm text-slate-500 mt-1">Instance-wide configuration for all users.</p>
                        ...
                        <form class="mt-6 space-y-6" method="POST" action="{{ route('admin.settings.store') }}">
                            ...
                            <div class="rounded-xl ring-1 ring-slate-900/5 p-5">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-slate-500 mt-0.5">When enabled, providers marked as "external" can receive document text for analysis.</p>
                                    </div>
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                                <div class="rounded-xl ring-1 ring-slate-900/5 p-5">
                                    ...
                                    <p class="text-xs text-slate-500 mt-2">Auto-delete uploads and exports after this many days.</p>
                                </div>
                                <div class="rounded-xl ring-1 ring-slate-900/5 p-5">
                                    ...
                                    <p class="text-xs text-slate-500 mt-2">Default: 104,857,600 (100 MB)</p>
                                </div>
                                <div class="rounded-xl ring-1 ring-slate-900/5 p-5">
                                    ...
                                    <p class="text-xs text-slate-500 mt-2">Controls verbosity of application logs.</p>
                                </div>
                            </div>
                            ...
                            <table class="min-w-full text-sm">
                                <thead class="bg-slate-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">When</th>
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @forelse(($auditEvents ?? collect()) as $ev)
                                        <tr class="hover:bg-slate-50/50">
                                            ...
                                            <td class="px-4 py-3 text-slate-500 text-xs whitespace-nowrap">{{ $ev->occurred_at?->diffForHumans() ?? $ev->created_at->diffForHumans() }}</td>
                                            <td class="px-4 py-3 text-slate-500 text-xs">{{ $ev->actor_user_id ? '#'.$ev->actor_user_id : '-' }}</td>
                    ...
                    @elseif($tab === 'observability')
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div class="lg:col-span-2">
                                <h3 class="text-base font-semibold text-slate-900 mb-4">Analysis Runs</h3>
                                <div class="space-y-3">
                                    @forelse(($analysisRuns ?? collect()) as $run)
                                        <div class="rounded-xl bg-slate-50 ring-1 ring-slate-900/5 p-4">
                                            <div class="flex items-start justify-between gap-4">
                                                <div class="flex-1">
                                                    <div class="flex items-center gap-2">
                                                        <span class="font-medium text-slate-900 text-sm">Run #{{ substr($run->uuid, 0, 8) }}</span>
                                                        <span @class([
                                                            'badge text-xs',
                                                            'badge-green' => $run->status === 'completed',
                                                            'badge-red' => $run->status === 'failed',
                                                            'badge-blue' => $run->status === 'running',
                                                            'badge-gray' => !in_array($run->status, ['completed', 'failed', 'running']),
                                                        ])>{{ $run->status }}</span>
                                                    </div>
                                                    ...
                                                    @if($run->error)
                                                        <div class="alert alert-error mt-2 text-xs">{{ $run->error }}</div>
                                                    @endif
                                                </div>
                                                <div class="text-xs text-slate-400 whitespace-nowrap text-right">
                                                    <div>{{ $run->created_at?->diffForHumans() ?? '' }}</div>
                                                    @if($run->created_by_user_id)
                                                        <div class="mt-0.5">user #{{ $run->created_by_user_id }}</div>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div>
                                ...
                                <div class="rounded-xl ring-1 ring-slate-900/5 overflow-hidden">
                                    ...
                                    @endphp
                                    @forelse(($analysisUsageByProvider ?? collect()) as $providerId => $count)
                                        @php $p = $providersById->get((int) $providerId); @endphp
                                        <div class="flex items-center justify-between px-5 py-3 border-b border-slate-100 last:border-b-0">
                                            <div>
                                                <div class="text-sm font-medium text-slate-900">{{ $p?->name ?? 'Unknown' }}</div>
                                                @if($p?->model) <div class="text-xs text-slate-500">{{ $p->model }}</div> @endif
                                            </div>
                                            <span class="text-lg font-bold text-slate-900">{{ (int) $count }}</span>
                                        </div>
                                    @empty
                                        <div class="p-8 text-center text-sm text-slate-500">No usage data yet.</div>
                                    @endforelse
                                </div>
                            </div>
                        </div>
