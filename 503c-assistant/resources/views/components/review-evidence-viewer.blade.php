...
<div class="rounded-xl ring-1 ring-slate-900/5 overflow-hidden">
    ...
    @else
        <div class="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
            <!-- Evidence List -->
            <div>
                <div class="px-4 py-2 text-xs font-medium text-slate-500 uppercase tracking-wider bg-slate-50/50">Citations</div>
                <div class="divide-y divide-slate-100 max-h-[400px] overflow-y-auto">
                    @foreach($fieldValue['evidence'] as $ev)
                        <a class="block px-4 py-3 transition-colors {{ $ev['is_active'] ? 'bg-indigo-50 border-l-2 border-indigo-500' : 'hover:bg-slate-50 border-l-2 border-transparent' }}" href="{{ route('projects.show', ['project' => $project->uuid, 'tab' => 'review', 'fv' => $fieldValue['id'] ?? 0, 'ev' => $ev['id']]) }}">
            ...
            <!-- Chunk Context -->
            <div>
                <div class="px-4 py-2 text-xs font-medium text-slate-500 uppercase tracking-wider bg-slate-50/50">Source Context</div>
                <div class="p-4">
                    ...
                    @if($fieldValue['highlighted_chunk'] !== '')
                        <div class="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed bg-slate-50 rounded-lg p-4 ring-1 ring-slate-900/5 max-h-[360px] overflow-y-auto">{!! $fieldValue['highlighted_chunk'] !!}</div>
                    @else
                        <p class="text-sm text-slate-500">Select an evidence citation to view its source context.</p>
                    @endif
