...
<div>
    <div class="flex items-center justify-between">
        ...
        <span class="text-xs font-medium text-slate-600" aria-label="{{ $stats['total'] - $stats['missing'] }} of {{ $stats['total'] }} fields complete">{{ $stats['total'] - $stats['missing'] }}/{{ $stats['total'] }}</span>
    </div>
...
<div class="mt-3 rounded-xl ring-1 ring-slate-900/5 divide-y divide-slate-100 max-h-[70vh] overflow-y-auto" role="list" aria-label="Protocol fields">
    @foreach($fieldValues as $fv)
        <a
            ...
            role="listitem"
            aria-current="{{ $fv['is_selected'] ? 'true' : 'false' }}"
        ...
        >
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1">
                    ...
                    <div class="text-xs text-slate-600 truncate mt-0.5">{{ $fv['key'] }}</div>
                </div>
                <div class="flex flex-col items-end gap-1">
                    ...
                    ]) aria-label="Status: {{ $fv['status'] }}">{{ $fv['status'] }}</span>
                    @if($fv['evidence_count'] > 0)
                        <span class="text-xs text-slate-500" aria-label="{{ $fv['evidence_count'] }} evidence items">{{ $fv['evidence_count'] }} ev</span>
                    @endif
                </div>
            </div>
        </a>
