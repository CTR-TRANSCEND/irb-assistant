...
test('post-login project tabs and admin tabs work', async ({ page }) => {
  ...
  const projectTabs: Array<{ name: string; tab: string; anchorText: string }> = [
    ...
    { name: 'Review', tab: 'review', anchorText: 'Review suggestions' },
    { name: 'Questions', tab: 'questions', anchorText: 'Guided questionnaire' },
    { name: 'Export', tab: 'export', anchorText: 'Export Protocol' },
    { name: 'Activity', tab: 'activity', anchorText: 'Audit trail' },
  ];

  for (const t of projectTabs) {
    const link = page.getByRole('link', { name: t.name, exact: true });
  ...
  for (const t of adminTabs) {
    ...
    await expect(page.locator('div.p-6').getByText(t.anchorText, { exact: false })).toBeVisible();
  }
});
