...
- Laravel application located at `/home/juhur/PROJECTS/project_IRB-assist/irb-assistant`
...
sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
  -keyout /etc/ssl/private/irb-assistant-selfsigned.key \
  -out /etc/ssl/certs/irb-assistant-selfsigned.crt \
  -subj "/C=US/ST=State/L=City/O=Organization/CN=irb.example.org"

# Set proper permissions
sudo chmod 600 /etc/ssl/private/irb-assistant-selfsigned.key
sudo chmod 644 /etc/ssl/certs/irb-assistant-selfsigned.crt
```
...
sudo cp /home/juhur/PROJECTS/project_IRB-assist/ops/apache/irb-assistant-production.conf \
  /etc/apache2/sites-available/irb-assistant.conf

# Update ServerName to match your actual domain or IP
sudo nano /etc/apache2/sites-available/irb-assistant.conf
...
```bash
# Enable the site
sudo a2ensite irb-assistant.conf
...
cd /home/juhur/PROJECTS/project_IRB-assist/irb-assistant
nano .env
...
sudo nano /etc/systemd/system/irb-assistant.service
```
...
User=juhur
Group=juhur
WorkingDirectory=/home/juhur/PROJECTS/project_IRB-assist/irb-assistant
ExecStart=/usr/bin/php /home/juhur/PROJECTS/project_IRB-assist/artisan serve --host=127.0.0.1 --port=8000
Restart=always
RestartSec=10
...
sudo systemctl enable irb-assistant
sudo systemctl start irb-assistant
sudo systemctl status irb-assistant
```
...
sudo tail -f /var/log/apache2/irb-assistant-error.log
```
...
ls -la /etc/ssl/certs/irb-assistant-selfsigned.crt
ls -la /etc/ssl/private/irb-assistant-selfsigned.key

# Check certificate details
openssl x509 -in /etc/ssl/certs/irb-assistant-selfsigned.crt -text -noout
```
...
sudo chmod -R 755 /home/juhur/PROJECTS/project_IRB-assist/irb-assistant

# Ensure storage and cache are writable
chmod -R 775 /home/juhur/PROJECTS/project_IRB-assist/storage
chmod -R 775 /home/juhur/PROJECTS/project_IRB-assist/bootstrap/cache
```
...
sudo a2ensite irb-assistant.conf
sudo a2dissite irb-assistant.conf
...
- Apache config: `/etc/apache2/sites-available/irb-assistant.conf`
- SSL Certificate: `/etc/ssl/certs/irb-assistant-selfsigned.crt`
- SSL Key: `/etc/ssl/private/irb-assistant-selfsigned.key`
- Error logs: `/var/log/apache2/irb-assistant-error.log`
- Access logs: `/var/log/apache2/irb-assistant-access.log`
