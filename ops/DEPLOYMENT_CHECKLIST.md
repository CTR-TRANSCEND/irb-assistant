...
```bash
git grep -nI -E '(password|passwd|secret|token|api[_-]?key|DB_PASS=|DB_PASSWORD=|APP_KEY=base64:|AKIA[0-9A-Z]{16}|-----BEGIN [A-Z ]*PRIVATE KEY-----)' -- ops/
```
...
- [ ] Copy `/ops/apache/irb-assistant-production.conf` to Apache sites-available
...
- [ ] Enable site: `sudo a2ensite irb-assistant-production.conf`
...
### Cron Jobs
- [ ] Copy `/ops/cron/irb-assistant.crontab.example` to crontab
...
|------|---------|-----------------|
| `/ops/apache/irb-assistant-production.conf` | Apache vhost | Update domain, cert paths, include path |
...
| `/ops/cron/irb-assistant.crontab.example` | Scheduler | Update paths, add to crontab |
