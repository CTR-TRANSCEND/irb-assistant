...
- [x] Scheduling also documented via cron example: `ops/cron/irb-assistant.crontab.example`.
...
`FileEncryptionService` supports multiple keys via the `id:base64key` format. Old uploads/exports are decrypted by lookup on the embedded key id; new writes use whichever key id is set as `IRB_FILE_ENCRYPTION_ACTIVE_KEY_ID`.

**Format note:** the value after the colon is the raw base64-encoded 32-byte key — there is NO `base64:` prefix. `parseKeyring` splits on the first `:` (limit 2), then calls `base64_decode($value, true)` in strict mode. A `base64:` prefix would not survive `base64_decode` (because `:` is not in the base64 alphabet) and the key would be silently discarded, disabling encryption.

1. Generate a fresh 32-byte key:
   ```bash
   php -r 'echo base64_encode(random_bytes(32))."\n";'
   ```
2. Append it to `IRB_FILE_ENCRYPTION_KEYS` with a new id (e.g., `2025a`). The old and new key VALUES are themselves base64-encoded — no extra prefix:
   ```
   IRB_FILE_ENCRYPTION_KEYS="2024a:<old-base64-44-chars>,2025a:<new-base64-44-chars>"
   ```
...
### Configuration access pattern (env() vs config())

All `IRB_*` environment variables are read **only** via `config('irb.*')` (see `config/irb.php`). Application code MUST NOT call `env()` directly outside config files. Reason: when `php artisan config:cache` is in effect on production, Laravel skips parsing `.env` and `env()` returns `null`, silently breaking any runtime code that relies on it.

- ✅ `config('irb.file_encryption_keys')` — works in both cached and non-cached modes
- ❌ `env('IRB_FILE_ENCRYPTION_KEYS')` — silently returns `null` after `config:cache`

If you add a new IRB_* env var: add it to `config/irb.php` with a sensible default, and read it via `config('irb.your_key')`.

### Production .env file ownership

The production `.env` file MUST be readable by the PHP-FPM service user. On a typical install where the FPM user is e.g. `irb_app` and the deploy user is e.g. `deployer`, both should share a group (e.g. `webapps`) and the file mode should be `640` (`deployer:webapps`).

Symptoms of a wrong group: `php artisan config:cache` works (because the deploy user runs it), but clearing the cache or running migrations causes 500s with "No application encryption key has been specified". Recovery: `chgrp webapps .env && chmod 640 .env`.

Sed in-place edits (`sed -i`) reset group ownership to the editing user's primary group. After any direct `.env` edit on production, re-verify ownership with `ls -la`.

Reference incident: 2026-05-10 (PROJECT_LOG.md).
