...
class AdminController extends Controller
{
    public function index(Request $request, SettingsService $settings): View
    {
        ...
        $data = [];
        if ($tab === 'users') {
            // SPEC-AUTH-001 REQ-AUTH-030: pending-count badge + split collections for the
            // Users tab. Variable names are the exact contract expected by admin._users-tab.
            $data['pending_count'] = User::query()->where('is_approved', false)->count();
            $data['pendingUsers'] = User::query()
                ->where('is_approved', false)
                ->orderByDesc('created_at')
                ->get();
            $data['allUsers'] = User::query()->orderBy('email')->limit(200)->get();
        } elseif ($tab === 'providers') {
            $data['providers'] = LlmProvider::query()->orderBy('name')->get();
            $editId = $request->query('edit');
            $data['editingProvider'] = $editId !== null
                ? LlmProvider::query()->find($editId)
                : null;
            ...
            $data['settings'] = [
                ...
                'retention_days' => $settings->int('retention_days', (int) config('irb.retention_days', 14)),
                'max_upload_bytes' => $settings->int('max_upload_bytes', (int) config('irb.max_upload_bytes', 104857600)),
                'logging_level' => $settings->string('logging_level', (string) config('logging.default', 'stack')),
            ];
        } elseif ($tab === 'audit') {
