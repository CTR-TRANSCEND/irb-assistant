...
class TemplateService
{
     ...
     *
     * Maps form_code -> [template_name, bundled_filename, storage_subdir].
     * Each entry is installed by ensureAllTemplatesInstalled().
     *
     * Outstanding #51: keys MUST match {@see FormCode}::values(). The migration
     * ENUM at database/migrations/2026_05_12_080000_add_form_code_to_projects_table.php
     * is also frozen to this allowlist. A guard test in
     * tests/Unit/FormCodeAllowlistTest.php asserts the two stay in sync.
     */
    public const FORM_TEMPLATES = [
        'hrp503' => ['name' => 'HRP-503', 'file' => 'HRP-503.docx', 'subdir' => 'hrp503'],
        'hrp503c' => ['name' => 'HRP-503c', 'file' => 'HRP-503c.docx', 'subdir' => 'hrp503c'],
        'hrp398' => ['name' => 'HRP-398', 'file' => 'HRP-398.docx', 'subdir' => 'hrp398'],
    ];

    public function ensureDefaultTemplateInstalled(?int $uploadedByUserId = null): TemplateVersion
    {
        // Backward-compatible alias: install HRP-503c (the historical default)
        // and return its TemplateVersion. New callers should prefer
        // ensureAllTemplatesInstalled() or ensureTemplateForFormCode().
        return $this->ensureTemplateForFormCode('hrp503c', $uploadedByUserId);
    }

    /**
     * Install (or refresh) the bundled template for every supported form_code.
     * Idempotent — safe to call on every request.
     *
     * @return array<string, TemplateVersion> keyed by form_code
     */
    public function ensureAllTemplatesInstalled(?int $uploadedByUserId = null): array
    {
        $installed = [];
        foreach (array_keys(self::FORM_TEMPLATES) as $formCode) {
            $installed[$formCode] = $this->ensureTemplateForFormCode($formCode, $uploadedByUserId);
        }
    ...
    /**
     * Install (or refresh) the bundled template for a single form_code.
     * Idempotent.
     */
    public function ensureTemplateForFormCode(string $formCode, ?int $uploadedByUserId = null): TemplateVersion
    {
        if (! isset(self::FORM_TEMPLATES[$formCode])) {
            throw new \InvalidArgumentException('Unknown form_code: '.$formCode);
        }

        $spec = self::FORM_TEMPLATES[$formCode];
        $sourcePath = base_path('resources/templates/'.$spec['file']);
        if (! is_file($sourcePath)) {
            throw new \RuntimeException('Bundled template not found: '.$sourcePath);
        }

        $sha256 = hash_file('sha256', $sourcePath);
        if ($sha256 === false) {
            throw new \RuntimeException('Failed to hash bundled template: '.$sourcePath);
        }
        ...
        $storageDisk = 'local';
        $storagePath = "templates/{$spec['subdir']}/{$sha256}.docx";

        $bytes = file_get_contents($sourcePath);
        if ($bytes === false) {
            throw new \RuntimeException('Failed to read bundled template: '.$sourcePath);
        }
    ...
    public function seedFieldDefinitionsFromControls(
        ...
        foreach ($controls as $ctrl) {
            ...
            // SPEC-IRB-CLEANUP-001: synthetic SDT-derived rows are no longer
            // surfaced as draftable fields. Their labels are unreliable OCR-grade
            // context fragments harvested by guessLabel() from Word XML run
            // segmentation. Curated hrp503.*/hrp503c.* keys never start with
            // 'ctrl_' so they are unaffected by this gate.
