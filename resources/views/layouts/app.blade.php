...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        {{-- Inter is self-hosted via @fontsource/inter (Vite-bundled). The
             ...
             dependency that conflicted with REQ-UI-017 and served stale
             font versions after the M7 self-host landed. --}}
    ...
    <body class="font-sans antialiased dark:bg-slate-900">
        ...
        @if(session('error'))
        <script>
        document.addEventListener('alpine:initialized', () => Alpine.store('toast').add(@json(session('error')), 'error'));
        </script>
        @endif
