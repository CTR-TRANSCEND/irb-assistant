{{-- Question type: text_short — single-line short text (max 80 chars) --}}
@php
...
<form method="POST" action="{{ route('submissions.answers.update', $routeArgs) }}" class="flex gap-2" x-data="{ loading: false }" @submit="loading = true">
    ...
    <button type="submit"
        ...
        <span x-show="loading" x-cloak class="flex items-center gap-1"><span class="spinner spinner-sm" aria-hidden="true"></span></span>
    </button>
</form>
