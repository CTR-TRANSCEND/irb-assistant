...
class LlmChatServiceTest extends TestCase
{
    ...
    public function test_chat_throws_on_malformed_response(): void
    {
        ...
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage('LLM response missing choices[0]');

        $this->service->chat($provider, $this->messages());
    }

    // ------------------------------------------------------------------
    // REQ-LLM-023: response-shape robustness for reasoning models
    // ------------------------------------------------------------------

    public function test_chat_returns_reasoning_content_when_content_is_null(): void
    {
        $provider = $this->makeProvider([
            ...
            'api.example.test/*' => Http::response([
                'choices' => [
                    ['message' => [
                        'role' => 'assistant',
                        'content' => null,
                        'reasoning_content' => 'OK from gpt-oss reasoning channel',
                    ]],
                ],
            ], 200),
        ]);
        ...
        $this->assertSame('OK from gpt-oss reasoning channel', $result);
    }

    public function test_chat_returns_reasoning_when_content_and_reasoning_content_both_empty(): void
    {
        $provider = $this->makeProvider([
            'name' => 'reasoning-fallback-provider',
            'base_url' => 'https://api.example.test/v1',
        ]);
            ...
            'api.example.test/*' => Http::response([
                'choices' => [
                    ['message' => [
                        'role' => 'assistant',
                        'content' => '',
                        'reasoning' => 'DeepSeek-R1 style reasoning text',
                    ]],
                ],
            ], 200),
        ]);
        ...
        $this->assertSame(
            'DeepSeek-R1 style reasoning text',
            $this->service->chat($provider, $this->messages())
        );
    }

    public function test_chat_returns_empty_string_for_tool_calls_only_response(): void
    {
        $provider = $this->makeProvider([
            'name' => 'tool-calls-provider',
            'base_url' => 'https://api.example.test/v1',
        ]);
        ...
        $this->assertSame('', $this->service->chat($provider, $this->messages()));
    }

    public function test_chat_returns_legacy_text_field_when_message_missing(): void
    {
        $provider = $this->makeProvider([
        ...
        Http::fake([
            'api.example.test/*' => Http::response([
                'choices' => [
                    ['text' => 'legacy completion-style text'],
                ],
            ], 200),
        ]);

        $this->assertSame(
            'legacy completion-style text',
            $this->service->chat($provider, $this->messages())
        );
    }

    public function test_chat_works_when_base_url_omits_v1_suffix(): void
    {
        $provider = $this->makeProvider([
            'name' => 'lmstudio-bare-base-url',
            'provider_type' => 'lmstudio',
            'base_url' => 'http://lmstudio.test:1234',
            'api_key' => null,
        ]);

        Http::fake([
            'lmstudio.test:1234/*' => Http::response([
                'choices' => [
                    ['message' => ['content' => 'OK from LM Studio']],
                ],
            ], 200),
        ]);
        ...
        $this->assertSame('OK from LM Studio', $result);

        Http::assertSent(function (\Illuminate\Http\Client\Request $request) {
            return $request->url() === 'http://lmstudio.test:1234/v1/chat/completions';
        });
    }

    public function test_chat_works_when_base_url_already_has_v1_suffix(): void
    {
        $provider = $this->makeProvider([
            'name' => 'lmstudio-with-v1',
            'provider_type' => 'lmstudio',
        ...
        Http::fake([
            'lmstudio.test:1234/*' => Http::response([
                'choices' => [
                    ['message' => ['content' => 'OK with explicit v1']],
                ],
            ], 200),
        ]);
        ...
        // Must not double-up to /v1/v1/chat/completions
        Http::assertSent(function (\Illuminate\Http\Client\Request $request) {
            return $request->url() === 'http://lmstudio.test:1234/v1/chat/completions';
        });
    }

    public function test_chat_surfaces_server_error_message_when_200_with_error_body(): void
    {
        $provider = $this->makeProvider([
            'name' => 'wrong-endpoint-provider',
            'base_url' => 'http://lmstudio.test:1234',
            'api_key' => null,
        ]);
        ...
        try {
            ...
            $this->assertStringContainsString('200 server error', $e->getMessage());
            $this->assertStringContainsString('Unexpected endpoint or method', $e->getMessage());
        }
    }

    public function test_normalize_base_url_strips_trailing_slash_and_v1(): void
    {
        $this->assertSame('http://h.test:1234', \App\Services\LlmChatService::normalizeBaseUrl('http://h.test:1234'));
        $this->assertSame('http://h.test:1234', \App\Services\LlmChatService::normalizeBaseUrl('http://h.test:1234/'));
        $this->assertSame('http://h.test:1234', \App\Services\LlmChatService::normalizeBaseUrl('http://h.test:1234/v1'));
        $this->assertSame('http://h.test:1234', \App\Services\LlmChatService::normalizeBaseUrl('http://h.test:1234/v1/'));
        $this->assertSame('https://api.openai.com', \App\Services\LlmChatService::normalizeBaseUrl('https://api.openai.com/v1'));
        // Custom paths preserved (not /v1)
        $this->assertSame('https://h.test/custom', \App\Services\LlmChatService::normalizeBaseUrl('https://h.test/custom/v1'));
    }

    public function test_chat_throws_descriptive_error_when_no_known_shape_matches(): void
    {
        $provider = $this->makeProvider([
            'name' => 'unknown-shape-provider',
            'base_url' => 'https://api.example.test/v1',
        ]);
        ...
        Http::fake([
            'api.example.test/*' => Http::response([
                'choices' => [
                    ['message' => ['role' => 'assistant', 'novel_field' => 'mystery']],
                ],
            ], 200),
        ]);
        ...
        try {
            ...
            $this->assertStringContainsString('Unexpected LLM response shape', $e->getMessage());
            $this->assertStringContainsString('role', $e->getMessage());
            $this->assertStringContainsString('novel_field', $e->getMessage());
        }
    }
